...
// ✅ 원본 API들
import {
    fetchGyms,              // 훅 내부에서 사용 중이지만 원본 보존을 위해 임포트 유지 가능
    fetchGymDetail,
    toggleFavorite,
    fetchGymReviews,
} from "../api/gymApi.jsx";
import { fetchTrainerDetail, fetchTrainerReviews } from "../../trainers/api/trainerApi.jsx";
...
const MapComponent = () => {
    ...
    const {
        mapRef, circleRef, userMarkerRef,
        isMapReady, isGeometryReady,
        userPos, setUserPos, userPosXY, setUserPosXY,
        moveToFixedLocation, getUserLatLng
    } = useKakaoMap();
    ...
    useRadiusCircle({ mapRef, circleRef, userPos, radius, showRadius, onlyInRadius });
    ...
    useGymMarkers({ mapRef, isMapReady, filteredGyms, activeInfoWindow, setActiveInfoWindow });
    ...
    // 길찾기
    const { openKakaoRouteToSelected } = useDirections({ selectedGym, userPos, userPosXY });
    ...
    const selectFromList = async (g) => {
        ...
        try {
            ...
            setSelectedError("");
            const detail = await fetchGymDetail(g.gymNo, loginState?.memberNo || null);
    ...
    const openTrainerPanel = async (trainerNo) => {
        try {
            ...
            const reviews = Array.isArray(reviewsRaw) ? reviewsRaw : reviewsRaw?.content || [];
            ...
            <div className="map-layout">
                ...
                <aside className="sidebar">
                    ...
                    <div className="sidebar__section">
                        <div className="sidebar__search">
                            ...
                            />
                            {searchTerm && (
                                <button className="sidebar__search-clear" onClick={() => setSearchTerm("")}>❌</button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="sidebar__section sidebar__filters">
                        ...
                        <div className="row">
                            <input
                                type="number"
                                value={radius}
                                onChange={(e) => setRadius(Math.max(Number(e.target.value || 0), 10))}
                                min={10}
                                step={10}
                            />
                            <span>m</span>
                            <button className="btn--line" onClick={() => setShowRadius((v) => !v)}>
                                {showRadius ? "반경 숨기기" : "반경 표시"}
                            </button>
                        </div>
                        <button className="btn btn--green" style={{ marginTop: 8, width: "100%" }} onClick={moveToFixedLocation}>
                            📍 고정 위치로 이동
                        </button>
                        <button
                            className="btn btn--primary"
                            style={{ marginTop: 8, width: "100%" }}
                            onClick={() => userPos && mapRef.current && mapRef.current.setCenter(userPos)}
                        >
                            📍 내 위치로
                        </button>
                    </div>
                    ...
                    <div className="sidebar__list">
                        {filteredGyms.length === 0 && <div style={{ color: "#9ca3af" }}>검색 결과가 없습니다.</div>}
                        {sortedGyms.map((g) => (
                            <div key={g.gymNo} className="card" onClick={() => selectFromList(g)}>
                                <div className="card__title">{g.name}</div>
                                <div className="card__addr">{g.address}</div>
                                <div className="card__coords">({g.lat.toFixed(4)}, {g.lng.toFixed(4)}) · {Math.round(g._dist)}m</div>
                            </div>
                        ))}
                    </div>
                </aside>
                ...
                <div className="map-area">
                    ...
                    <div className="summary">
                        {selectedLoading && <div className="summary__box">요약 불러오는 중...</div>}
                        {selectedError && <div className="summary__box summary__box--error">{selectedError}</div>}

                        {selectedGym && !selectedLoading && !selectedError && (
                            <div className={`summary__box ${showDetail ? "summary__box--scroll" : ""}`}>
                                {/* 헤더 */}
                                <div className="summary__head">
                                    <div className="summary__title">{safe(selectedGym.title || selectedGym.name, "이름 없음")}</div>
                                    <div>
                                        <button
                                            className="btn btn--primary"
                                        ...
                                        <button
                                            ...
                                            onClick={() => { setSelectedGym(null); setShowDetail(false); setActivePanel(null); }}
                                            title="닫기"
                                        >
                                            ✖
                                        </button>
                                ...
                                {!showDetail && (
                                    <>
                                        <div className="summary__meta">
                                            평점: {Number.isFinite(selectedGym.rate) ? Number(selectedGym.rate).toFixed(1) : "0.0"} / 5
                                            {typeof selectedGym.favoriteCount === "number" ? ` · ⭐ ${selectedGym.favoriteCount}` : ""}
                                        </div>
                                        <div className="summary__line">📍 {safe(selectedGym.address)}</div>
                                        <div className="summary__line">📞 {safe(selectedGym.phoneNumber)}</div>
                                        <div className="summary__line">🕒 {safe(selectedGym.openingHours)}</div>
                                        <div className="summary__desc">{clip(selectedGym.content, 120)}</div>
                                        <div className="summary__actions">
                                            <button className="btn btn--primary" style={{ flex: 1 }} onClick={() => setShowDetail(true)}>
                                                상세 보기
                                            </button>
                                        </div>
                                    </>
                                ...
                                {showDetail && (
                                    <>
                                        ...
                                        <div className="summary__actions">
                                            <button className="btn btn--line" onClick={() => openReviewPanel(selectedGym.gymNo)}>
                                                💬 리뷰 전체 보기
                                            </button>
                                            <button
                                                className="btn btn--primary"
                                                onClick={() => navigate("/gyms/1/purchase")}
                                                title="결제 페이지로 이동"
                                            >
                                                등록하기
                                            </button>
                    ...
                    {activePanel === "review" && selectedGym && (
                        <div className="summary" style={{ left: "auto", right: "16px" }}>
                            <div className="summary__box summary__box--scroll">
                                <div className="summary__head">
                                    <div className="summary__title">{selectedGym.title || selectedGym.name} 리뷰</div>
                                    <button className="summary__close" onClick={closePanel}>✖</button>
                                </div>
                    ...
                    {activePanel === "trainer" && (
                        <div className="summary" style={{ left: "auto", right: "16px" }}>
                            <div className="summary__box summary__box--scroll">
                                <div className="summary__head">
                                    <div className="summary__title">{trainerDetail?.name || "트레이너"}</div>
                                    <button className="summary__close" onClick={closePanel}>✖</button>
                                </div>

                                {trainerLoading && <div>불러오는 중...</div>}
                                {trainerError && !trainerLoading && (
                                    <div className="summary__box summary__box--error">{trainerError}</div>
                                )}

                                {!trainerLoading && !trainerError && trainerDetail && (
                                    <>
                                        {Number.isFinite(trainerDetail.rate) && (
                                            <div className="summary__meta">평균 평점: {Number(trainerDetail.rate).toFixed(1)} / 5</div>
                                        )}
                                        {trainerDetail.specialty && (
                                            <div className="summary__line">전문 분야: {trainerDetail.specialty}</div>
                                        )}
                                        {(trainerDetail.gym?.gymNo || trainerDetail.gymNo) && (
                                            <div className="summary__line">
                                                ...
                                                <button
                                                    className="btn--line"
                                                    onClick={() => navigate(`/gyms/${trainerDetail.gym?.gymNo || trainerDetail.gymNo}`)}
                                                >
                                        ...
                                        {trainerDetail.description && (
                                            <div className="summary__desc" style={{ whiteSpace: "pre-wrap" }}>
                                                {trainerDetail.description}
                                            </div>
                                        )}
                                        <div className="summary__line" style={{ marginTop: 12, fontWeight: 700 }}>
                                            최근 리뷰
                                        </div>
                                            ...
                                            trainerReviews.slice(0, 3).map((r) => (
                                                <div key={r.reviewNo} className="detail-review-card">
                                                    <div style={{ fontWeight: 600 }}>
                                                        {(r.writerNickname || r.writerName || "익명")} · ★{Number(r.score).toFixed(1)}
                                                    </div>
                                                    <div style={{ marginTop: 4 }}>{r.comment}</div>
                                                </div>
                                            ))
                                        ) : (
                                            <div style={{ color: "#888" }}>등록된 리뷰가 없습니다.</div>
                                        )}
                                        <div className="summary__actions">
                                            <button
                                                className="btn btn--primary"
                                                style={{ flex: 1 }}
                                                onClick={() => navigate(`/gyms/trainers/${trainerDetail.trainerNo}/reviews`)}
                                            >
                                                리뷰 더 보기/작성
                                            </button>
                                        </div>
