...
const MultChatRoomList = () => {
  ...
  // 내 채팅방 로드 후, 소속 유지용 keepalive (임시퇴장/새로고침 후에도 참가자 수 유지)
  useEffect(() => {
    if (!isWebSocketConnected) return;
    // myRooms가 갱신될 때, 참여 중인 방에 대해 서버가 참가자 수를 유지/재브로드캐스트 하도록 유도
    // (서버는 임시퇴장을 실제 퇴장으로 처리하지 않으므로, 이 호출은 멱등적)
    myRooms
        ...
        <div className="flex items-center justify-between text-sm text-gray-500">
          <div className="flex items-center space-x-4">
            ...
            >
              ...
              {realtimeInfo && (
                <span className="ml-1 text-xs text-teal-500" title="실시간 업데이트됨">●</span>
              )}
            </span>
