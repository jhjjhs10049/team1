...
public interface MultChatRoomParticipantRepository extends JpaRepository<MultChatRoomParticipant, Long> {
    ...
    // 특정 채팅방의 활성 참가자 목록 조회 (Member 정보 즉시 로딩)
    ...
    // 특정 멤버의 특정 채팅방 참가 정보 조회 (Member 정보 즉시 로딩)
    @Query("SELECT p FROM MultChatRoomParticipant p " +
           "JOIN FETCH p.member " +
    ...
    // 특정 멤버의 특정 채팅방 활성 참가 정보 조회 (Member 정보 즉시 로딩)
    @Query("SELECT p FROM MultChatRoomParticipant p " +
           "JOIN FETCH p.member " +
    ...
    // 특정 멤버가 참가 중인 채팅방의 참가자 정보 목록 조회 (Member 정보 즉시 로딩)
    @Query("SELECT p FROM MultChatRoomParticipant p " +
           "JOIN FETCH p.member " +
